...
//유니티에서는 주로 XXXController , XXXManager 등의 구현에 해당 패턴을 자주 사용합니다.

//싱글톤 패턴은 static 키워드를 활용한 기능으로 전역 변수를 사용하게 되는 개념입니다.
//따라서 전역 변수의 문제인 메모리 부족 / 멀티 태스킹 환경에서 오류 발생 , 코드 리팩토링(수정)
//과정에서 해당 전역 변수 사용 중단 시 모든 모듈 수정 등의 단점을 그대로 가지게 됩니다.

//싱글톤 사용 시 적절한 상황 판단
//범용성이 적어서 특정 클래스에만 쓰이는 코드일 경우 싱글톤으로 구현하지 않고 그 클래스에 그 코드를 넣는게 더 이득입니다.
//유니티에서 제공되는 기능이 있다면 그 기능으로 해결하는 것이 더 나을 수 있습니다.

//장점 : 다른 클래스와는 독립적으로 유지됨.
//       객체 접근에 매우 용이합니다.(전역)
...
public class SingletonBasic : MonoBehaviour
{
    ...
    //객체를 생성하고 일반적인 경우에는 만들어진 객체를 return 하도록 설계합니다.
