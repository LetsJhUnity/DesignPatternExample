...
//제네릭 클래스
//클래스명<T>로 구성하면 T 자리에 타입을 넣을 수 있습니다.

//제네릭 제약 조건
/*where T : struct  => T는 null을 허용하지 않는 값의 형식
where T : class   => T는 참조(Reference) 형식
where T: new()    => 기본 생성자가 있어야 합니다. 다른 제약 조건과 같이 쓰일 경우 가장 마지막에 작성됩니다.
...
public class TSingleton<T> : MonoBehaviour where T : MonoBehaviour
{
    ...
    public static T Instance
    {
        get
        {
            ...
            //c#의 lock 기능
            //특정 블럭의 코드를 한번에 하나의 쓰레드만 실행할 수 있도록 처리해주는 도구
            ...
            lock(_Lock)
            {
                if (_instance == null)
                {
                    ...
                    if( _instance == null)
                    {
                        ...
                        //인스턴스 객체를 T를 컴포넌트로 추가한 sObj로 할당
